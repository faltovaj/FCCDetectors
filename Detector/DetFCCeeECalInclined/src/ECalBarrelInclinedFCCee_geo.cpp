...
namespace det {
static dd4hep::detail::Ref_t createECalBarrelInclinedFCCee(dd4hep::Detector& aLcdd,
  ...
  dd4hep::xml::DetElement cryoFrontWarm = cryostat.child(_Unicode(cryo_front_warm));
  dd4hep::xml::DetElement cryoFrontCold = cryostat.child(_Unicode(cryo_front_cold));
  ...
  double cryoThicknessFront = cryoFrontWarmDim.rmax() - cryoFrontWarmDim.rmin();
  dd4hep::xml::Dimension cryoFrontColdDim(cryoFrontCold.dimensions());
  ...
  lLog << MSG::DEBUG << "**** FCCee ECAL Inclined ****" << endmsg;
  ...
  if (cryoThicknessFront > 0) {
    ...
    dd4hep::Tube cryoFrontColdShape(cryoFrontColdDim.rmin(), cryoFrontColdDim.rmax(), cryoFrontColdDim.dz());
    ...
    if (cryoFrontSensitive) {
      ...
      cryoFrontWarmPhysVol.addPhysVolID("type", 1);
      lLog << MSG::INFO << "Cryostat front warm volume set as sensitive" << endmsg;
      cryoFrontColdVol.setSensitiveDetector(aSensDet);
    ...
    if (cryoBackSensitive) {
      ...
      cryoBackColdPhysVol.addPhysVolID("cryo", 1);
      cryoBackColdPhysVol.addPhysVolID("type", 3);
      lLog << MSG::INFO << "Cryostat back cold volume set as sensitive" << endmsg;
      cryoSolenoidVol.setSensitiveDetector(aSensDet);
      cryoSolenoidPhysVol.addPhysVolID("cryo", 1);
      cryoSolenoidPhysVol.addPhysVolID("type", 4);
      lLog << MSG::INFO << "Cryostat solenoid volume set as sensitive" << endmsg;
      cryoBackWarmVol.setSensitiveDetector(aSensDet);
      cryoBackWarmPhysVol.addPhysVolID("cryo", 1);
      cryoBackWarmPhysVol.addPhysVolID("type", 5);
    ...
    dd4hep::DetElement cryoFrontColdDetElem(caloDetElem, "cryo_front_cold", 0);
    cryoFrontColdDetElem.setPlacement(cryoFrontColdPhysVol);
    dd4hep::DetElement cryoBackColdDetElem(caloDetElem, "cryo_back_cold", 0);
    cryoBackColdDetElem.setPlacement(cryoBackColdPhysVol);
    dd4hep::DetElement cryoSolenoidDetElem(caloDetElem, "cryo_solenoid", 0);
    ...
    lLog << MSG::INFO << "ECAL services: front: rmin (cm) = " << cryoFrontColdDim.rmax() << " rmax (cm) = " <<  bathRmin << " dz (cm) = " << caloDim.dz() << endmsg;
    lLog << MSG::INFO << "ECAL services: back: rmin (cm) = " << bathRmax << " rmax (cm) = " << cryoBackColdDim.rmin() << " dz (cm) = " << caloDim.dz() << endmsg;
  ...
  lLog << MSG::INFO << "ECAL bath: material = " << activeMaterial << " rmin (cm) =  " << bathRmin
       << " rmax (cm) = " << bathRmax << " thickness in front of ECal (cm) = " << caloDim.rmin() - cryoFrontColdDim.rmax()
       << " thickness behind ECal (cm) = " << cryoBackColdDim.rmin() - caloDim.rmax() << endmsg;
...
DECLARE_DETELEMENT(EmCaloBarrelInclinedFCCee, det::createECalBarrelInclinedFCCee)
